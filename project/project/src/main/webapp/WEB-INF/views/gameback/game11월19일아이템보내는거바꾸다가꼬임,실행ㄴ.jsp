...
<script type="text/javascript">
	...
	$(function() {
		...
		ws.onmessage = function(event) {
			...
			if (ob.cmd == 'item') {
				var itemroot = ob.itemroot;
				for(var i=0;i<itemroot.length;i++){
					if(itemroot[i].eat){
						itemroot[i].apply=true;
					}
				}
			}
		};
		...
		var itemImg_4;
		var item; // 아이템 좌표
		var iteminfo; // 아이템 정보 저장
		var itemroot; // 아이템 먹고 보관하는곳
		var itemMax = 5;
		...
		function render() {
			...
			if (spacecnt % 2 == 0) {
				...
				for(var i=0;i<itemroot.lenght;i++){
					itemroot[i].eat=false;
				}
				/* database = new Array(); */
			}
			if (spacecnt % 5 == 0) {
			...
			if (!gameEnd ) {// && !realGameEnd
				setTimeout(function() {
					...
					if (itemroot[0].apply) { // 초기화
						
						enemyBalls = new Array();
						enemyBallsinfo = new Array();
						createEnemyBalls(10);
						itemroot[0].eat= false;
						itemroot[0].apply = false;
						
					}
					
					if (itemroot[1].apply) { // 30초동안 총알 2개
						if (item_twoweapon < 3) {
							item_twoweapon += 1;
							setTimeout(function() {
								item_twoweapon -= 1;
						...
						itemroot[1].eat= false;
						itemroot[1].eat = false;
					}
					
					
					if (itemroot[2].apply) { // 10개 생성
						createEnemyBalls(10);
						itemroot[2].apply = false;
					}
					
					if (itemroot[3].apply) { // 적스피드 빨라짐
						for (var i = 0; i < enemyBalls.length; i++) {
							enemyBallsinfo[i].speed = 15;
						}
						setTimeout(function() { // 되돌려주기
							for (var i = 0; i < enemyBalls.length; i++) {
								enemyBallsinfo[i].speed = 10;  // 랜덤수라.. 허허 10으로 고정시킴
							}
						}, 30000);
						itemroot[3].apply = false;
					}
			...
			for (var i = 0; i < item.length; i++) {
				...
				if (item[i].use) {
					...
					if (distance <= (iteminfo[i].width / 2 + (playerUnit.width / 2 - 10)) * (iteminfo[i].height / 2 + (playerUnit.height / 2 - 10))) {
						...
						if (i==0) {
							itemroot[0].apply = true;
						}
						else if (i==1) {
							itemroot[1].apply = true;
						}
