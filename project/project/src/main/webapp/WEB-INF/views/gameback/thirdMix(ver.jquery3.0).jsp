...
<head>
<meta charset="utf-8">
<title>thirdMix(ver.jquery3.0)</title>
<!-- 스코어구현 후 조건에 맞춰 아이템 생성됨 -->
...
<script type="text/javascript">
	...
	function jqueryOk() {
		...
		var playerBulletimg;
		var playerBullet;
		var playerBulletMax = 30;/* 최대 탄환갯수 */
		...
		function render() {
			...
			for (var i = 0; i < item.length; i++) {
				ctx.drawImage(itemimg, //Source Image
				0, 0, //X, Y Position on spaceShipSprit
				48, 46, //Cut Size from spaceShipSprit
	...
	console.log("생성됨"+score);
		...
		function calcitem() { 
			if (parseInt(score%5000) == 0) {
				createitem();
			}
			for (var i = 0; i < item.length; i++) {
				item[i].radians = item[i].angle * Math.PI / 180;
				item[i].x += Math.cos(item[i].radians) * item[i].speed;
				item[i].y += Math.sin(item[i].radians) * item[i].speed;

				if (item[i].x > canvas.width || item[i].x < 0) {
