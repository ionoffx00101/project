...
<head>
<meta charset="utf-8">
<title>thirdMix(ver.jquery3.2)</title>
<!-- 아이템 획득시 탄환 두개씩 나가는 기능을 넣음, 현재 그 값은 boolean 값이므로 탄환수를 더 늘리려면 해당 기능과 변수 조정 필요 -->
...
<script type="text/javascript">
	...
	function jqueryOk() {
		...
		var item;
		var itemMax = 5;
		var item_twoweapon =false; //탄환 공격 범위를 늘려주는 아이템임
		/* 내가 추가한 끝   */
		...
		function render() {
			...
			for (var i = 0; i < item.length; i++) {
				switch (i) {
				  case 0    : item[i].color='#ff0000';
				               break;
				  case 1   : item[i].color='#ffff00';
				               break;
				  case 2   : item[i].color='#ff00ff';
	               break;
				  case 3   : item[i].color='#00ffff';
	               break;
				  default    :  break;
				}
		...
		function useplayerBullet() {
			...
			else if (item_twoweapon) {
				playerBullet[playerBulletcnt].x = playerUnit.x + playerUnit.width / 2 +7;
				playerBullet[playerBulletcnt].y = playerUnit.y + playerUnit.height / 2;
				playerBullet[playerBulletcnt].speed = 3;
				playerBullet[playerBulletcnt].use = true;
				playerBulletcnt++;
