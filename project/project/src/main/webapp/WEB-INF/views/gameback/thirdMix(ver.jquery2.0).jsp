...
<head>
<meta charset="utf-8">
<title>thirdMix(ver.jquery2.0)</title>
<!-- 스페이스바 누르면 탄환 발사, 아군 탄환과 적 탄환 충돌 구현 -->
...
<script type="text/javascript">
	...
	function jqueryOk() {
		...
		canvasTemp = document.createElement("canvas"), scrollImg = new Image(), tempContext = canvasTemp
				.getContext("2d"), imgWidth = 0, imgHeight = 0, imageData = {}, canvasWidth = 352, canvasHeight = 1600, scrollVal = 0, speed = 2;
        ...
        var playerBulletimg;
        var playerBullet;
        var playerBulletMax=15;/* 최대 탄환갯수 */  
...
/* 6. 유닛의 과 탄환간의 거리를 재어 피격판정을내는 펑션, 피격의 경우 게임을 정지 시키기 위한 값을 돌려주고 그렇지 않을 경우 계속시킨다 */
		function checkHitPlayer() {
			...
			for (var i = 0; i < enemyBalls.length; i++) {
				var distanceX = (playerUnit.x + playerUnit.width / 2)
						- enemyBalls[i].x;
				var distanceY = (playerUnit.y + playerUnit.height / 2)
						- enemyBalls[i].y;
...
/*   8.게임 루프 펑션 */
		function render() {
			...
			imageData = tempContext.getImageData(0, canvasHeight - scrollVal,
			...
			imageData = tempContext.getImageData(0, 0, canvasWidth,
					canvasHeight - scrollVal);
			ctx.putImageData(imageData, 0, scrollVal, 0, 0, canvasWidth,
					imgHeight);
                          /* 배경 스크롤을 그려주는 부분 */
			...
			for(var i=0;i<playerBullet.length;i++)
			  {
			    /* ctx.fillStyle =playerBullet[i].color;
			    ctx.beginPath();
			    ctx.arc(playerBullet[i].x, playerBullet[i].y, playerBullet[i].radius, 0, Math.PI*2, true)
			    ctx.closePath();
			    ctx.fill(); */
		...
		function useplayerBullet() {
			...
			playerBullet[playerBulletcnt].x=playerUnit.x + playerUnit.width / 2;
			 ...
			 for(var i=0;i<playerBullet.length;i++)
			 {
			   if(playerBullet[i].use){/* 사용하는 공만 충돌판정 확인함 */
				   ...
				   for (var j = 0; j < enemyBalls.length; j++)
				   {
						...
						if (distance <= (enemyBalls[j].radius * enemyBalls[j].radius)) {
							nouseplayerBullet(i);
							enemyBalls[j].x=600;
							enemyBalls[j].y=600;
