...
<head>
<meta charset="utf-8">
<title>thirdMix(ver.jquery4.0)</title>
<!-- 게임 종료후  폭발 함 단순로직 4.1에서 많이 바뀔듯 백업용-->
...
<script type="text/javascript">
	...
	function jqueryOk() {
		...
		function render() {
			...
			/*  피격판정을 실시한다 */
			if(gameEnd){
				// render를 돌리면서 5초뒤에 졸료로 바꿀수도 있을거같다
				createExplosion();
			}
			if (!gameEnd) {
		...
		function playerExplosion(){
			...
			/* 폭발 그리는 부분 */
			ctx.drawImage(explosionimg,explosion.imgx,explosion.imgy,explosion.imgw,explosion.imgh, explosion.x,explosion.y,32,32);
			explosion.imgx += explosion.imgw;
			explosion.imgidx++;
			if(explosion.imgidx % 5==0) {
				explosion.imgx = 0;
				explosion.imgy += explosion.imgh;
			}

			if(explosion.imgidx>explosion.frame_cnt) clearInterval(explosiontimer);
		}
