...
<head>
<meta charset="utf-8">
<title>thirdMix(ver.jquery3.1)</title>
<!-- 스코어 삭제, 시간에 따라 아이템이 생성되고 아이템을 먹을 시 10초동안 탄환의 범위가 넓어짐, 아이템 벽에 튕김 -->
...
<script type="text/javascript">
	...
	function jqueryOk() {
		...
		var item;
		var itemMax = 3;
		var itemfn1 =15; //탄환 공격 범위를 늘려주는 아이템임
			...
			for (var i = 0; i < playerBullet.length; i++) {
				if (playerBullet[i].use) {/* 사용하는 공만 충돌판정 확인함 */
					...
					for (var j = 0; j < enemyBalls.length; j++) {
						...
						if (distance <= (enemyBalls[j].radius+itemfn1 * enemyBalls[j].radius+itemfn1)) {
							nouseplayerBullet(i);
							enemyBalls[j].x = 600;
							enemyBalls[j].y = 600;
			...
			for (var i = 0; i < item.length; i++) {
				if (item[i].use) {/* 사용하는 공만 충돌판정 확인함 */
					...
					if (distance <= (item[i].width / 2 + (playerUnit.width / 2 - 10)) * (item[i].height / 2 + (playerUnit.height / 2 - 10))) {
						...
						itemfn1+=150;
						console.log("아이템 먹음"+itemfn1);
						setTimeout(function name() {
							itemfn1-=150;
							console.log("시간끝남"+itemfn1);
						}, 10000)
