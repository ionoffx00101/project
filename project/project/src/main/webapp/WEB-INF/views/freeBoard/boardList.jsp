								...
								<table class="table">
									...
									<tbody>
										<c:forEach var="s" items="${list}">
											<tr>
												<td>${s.num}</td>
												<td class="colss"><a href="info?num=${s.num}">${s.title} </a></td>
												<td>${s.author}</td>
												<!-- 첨부파일 추가해주세요 -->
											</tr>
										</c:forEach>
											...
											<ul class="pagination pagination-lg">
												...
												<c:choose>
													<c:when test="${nvo.leftMore}">
														<li><a href="list?page=${nvo.links[0]-1}"> « </a></li>
													</c:when>
												</c:choose>
												...
												<c:forEach var="s" items="${nvo.links}">
													<c:choose>
														<c:when test="${nvo.currPage == s}">
															<li class="active"><a href="list?page=${s}">${s}</a></li>
														</c:when>
														<c:otherwise>
															<li><a href="list?page=${s}">${s}</a></li>
														</c:otherwise>
													</c:choose>
												</c:forEach>
												<!--  -->
												<c:choose>
													...
													<c:when test="${nvo.rightMore}">
														<li><a href="list?page=${nvo.links[1]+1}">»</a></li>
													</c:when>
												</c:choose>
											</ul>
										</div>
