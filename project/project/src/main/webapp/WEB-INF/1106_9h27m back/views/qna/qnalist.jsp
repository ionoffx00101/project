			...
			<div class="container">
				<div class="row">
					<div class="col-md-12 clearfix">
						<div class="box">
							<a href="qnapost"><button type="button" class="btn_postlnk">글쓰기</button></a>
							<br><br>
							<div class="content">
									...
									<table class="table">
										...
										<tbody>
											<c:forEach var="b" items="${list}">
												<tr>
													<td>${b.num}</td>
													<td class="colss"><a href="qnaread?num=${b.num}">
															${b.title} </a></td>
													<td>${b.author}</td>
													<td>${b.wdate}</td>
													<td>${b.hitcnt}</td>
							...
							<div class="searchform">
								<select name="category">
									<option>글제목</option>
									<option>글쓴이</option>
								</select><input type="text" class="form-control" placeholder="Search"> <input type="text" name="searchword">
								<button type="button" name="btn_search" class="btn_search">검색</button></div>
...
<Br><Br>
										...
										<c:when test="${search}">
											<c:choose>
												<c:when test="${navi.leftMore}">
													<a href="search?word=${word}&cate=${cate}&pnum=1"> [<<]
													</a>
          ...
          [<span style='color: red; font-size: 1.3em;'>${board}</span>]
       </c:when>
													<c:when test="${navi.currPage!=board}">
										...
										<c:when test="${search}">
											...
											<c:choose>
												<c:when test="${navi.rightMore}">
													<a
														href="search?word=${word}&cate=${cate}&pnum=${navi.linknum}">
														[>>] </a>
													<%-- ${navi.links.length-1}${navi.links().length-1}--%>
												</c:when>
											</c:choose>
