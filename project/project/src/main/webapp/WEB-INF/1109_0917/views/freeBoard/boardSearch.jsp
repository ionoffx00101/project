...
<table>
<tr>
	<td>번호</td><td>제목</td><td>작가</td>
</tr>
<c:forEach var="s" items="${list}">
<tr>
	<td>${s.num}</td><td><a href="info?num=${s.num}">${s.title}</td><td>${s.author}</td>
</tr>
</c:forEach>
</table>
...
<c:choose>
	<c:when test="${nvo.leftMore}">
		<a href="searchNavi?page=${nvo.links[0]-1}"> [<<] </a>
	</c:when>
</c:choose>
...
<c:otherwise>
	<a href="searchNavi?page=${s}">[${s}]</a> 
</c:otherwise>
</c:choose>
</c:forEach>
<c:choose>
	<c:when test="${nvo.rightMore}">
	<a href="searchNavi?page=${nvo.links[1]+1}"> [>>] </a>
	</c:when>
</c:choose>
</form>
</tiles:putAttribute>
